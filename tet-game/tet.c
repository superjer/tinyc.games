...
#define STB_IMAGE_IMPLEMENTATION // the vulkan lib includes stb_image (and SDL)
...
void do_events()
{
        while (SDL_PollEvent(&event)) switch (event.type)
        {
                ...
                case SDL_EVENT_GAMEPAD_ADDED:
                        SDL_OpenGamepad(event.gdevice.which);         break;
...
void setup()
{
        ...
        SDL_GetWindowSizeInPixels(vk.window, &win_x, &win_y);
        resize(win_x, win_y);
        audio_init();
        font_init();
        draw_setup();
...
void update_player()
{
        ...
        if (++p->idle_time >= (p->grounded ? 100 : speeds[MIN(MAX_SPEED, p->lines / 10)]))
        {
                move(0, 1, 1);
                p->idle_time = 0;
        }
