...
void draw_doors_lo(int offx, int offy)
{
        SDL_Rect src, dest;
...
void draw_doors_hi(int offx, int offy)
{
        SDL_Rect src, dest;
        int r = roomy*DUNW + roomx; // current room coordinate
        int *doors = rooms[r].doors;
...
void draw_stuff()
{
        ...
        if (inside)
        {
                SDL_RenderCopy(renderer, edgetex[0], NULL, &whole);
                draw_doors_lo(scrollx, scrolly);
        }
